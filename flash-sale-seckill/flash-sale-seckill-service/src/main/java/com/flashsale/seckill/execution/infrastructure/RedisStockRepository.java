...
public class RedisStockRepository implements StockRepository {
    ...
    private static final String DECR_LUA = "local stock = redis.call('get', KEYS[1]) "
        + "if not stock then return -1 end " + "stock = tonumber(stock) " + "if stock <= 0 then return -1 end "
    ...
    public long decrementStock(Long seckillGoodsId) {
        Long result = redisTemplate.execute(DECR_SCRIPT, Collections.singletonList(STOCK_KEY_PREFIX + seckillGoodsId));
