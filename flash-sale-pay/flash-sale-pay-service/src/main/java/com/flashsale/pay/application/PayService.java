 ...
 * 支付应用服务。
 *
 * <p>编排支付用例：创建支付、处理回调、查询结果。
 * 支付成功后通过 RocketMQ 发送支付通知消息，
 * 由订单服务消费完成订单状态变更。</p>
 */
@Slf4j
@Service
...
public class PayService {
     ...
     * 创建支付记录。
     *
     * <p>通过 {@link Payment#create} 领域工厂构建聚合根，
     * 支付渠道字符串转换为 {@link PayChannel} 枚举。</p>
     *
     ...
     * 处理第三方支付回调。
     *
     * <p>根据回调结果调用聚合根的 {@link Payment#markSuccess} 或
     * {@link Payment#markFail}（含状态机守卫校验）。
     * 支付成功后发送 RocketMQ 通知消息。</p>
     *
     * @param orderId 订单 ID
