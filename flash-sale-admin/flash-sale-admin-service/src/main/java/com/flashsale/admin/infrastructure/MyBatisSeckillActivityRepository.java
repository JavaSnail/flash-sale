...
public class MyBatisSeckillActivityRepository implements SeckillActivityRepository {
    ...
    /**
     * 数据对象 → 领域对象。
     * 将 Integer status 转换为 {@link ActivityStatus} 枚举，
     * 将 startTime/endTime 组合为 {@link TimeRange}。
     */
    private SeckillActivity toDomain(SeckillActivityDO d) {
        return SeckillActivity.reconstitute(
                d.getId(),
                d.getActivityName(),
                d.getGoodsId(),
                ...
                ActivityStatus.fromCode(d.getStatus() == null ? 0 : d.getStatus()),
                d.getCreateTime(),
                d.getUpdateTime()
        );
    }
    ...
    /**
     * 领域对象 → 数据对象。
     * 将 {@link ActivityStatus} 枚举转换为 Integer code，
     * 将 {@link TimeRange} 拆分为 startTime/endTime。
     */
